...
# [IQON : network Adapter Selection import : by knuth ]
...
class InfoBarChannelSelection:
		...
		self["ChannelSelectActions"] = HelpableActionMap(self, "InfobarChannelSelection",
			{
				...
				# [ IQON : PROV and SAT key define : by knuth
	...
	def keyChannelDownCheck(self):
		print "@@@ keyChannelDownCheck @@@"
		if config.usage.zap_with_ch_buttons.value:
			self.zapUp()
		else:
			self.openServiceList()
	# [IQON : define function PROV, SAT key
...
# [IQON : Remocon key function add. ( mediaplayer, rfmod, networksetup, aspectratio ) : by knuth
...
# [IQON : toggleaspectratio, showrfmod, showmediaplayer, shownetworksetup function defind : by knuth	
...
class InfoBarEPG:
		...
		self["EPGActions"] = HelpableActionMap(self, "InfobarEPGActions",
			{
				...
				"showInfobarOrEpgWhenInfobarAlreadyVisible": self.showEventInfoWhenNotVisible,
				# [ IQON : mytube function key mapping : by knuth
				"showYouTube": (self.showYouTube, _("Show YouTube...")), # current not activates
				# IQON : by knuth ]
			})
	...
	def showSingleEPG(self):
		# [ IQON : cooltvguide epg key mapping. : by knuth
	...
	def openEventView(self):
		# [ IQON : cooltvguide eventview key mapping. : by knuth
		if config.usage.try_cooltvguide.value:
			cooltvguidePath = "/usr/lib/enigma2/python/Plugins/Extensions/CoolTVGuide"
			import os
			if os.path.exists(cooltvguidePath + "/plugin.py") or os.path.exists(cooltvguidePath + "/plugin.pyo"):
	...
	def eventViewCallback(self, setEvent, setService, val): #used for now/next displaying
		...
		if len(epglist) > 1:
			...
			epglist[1]=tmp
			setEvent(epglist[0])
# [ IQON : shortcut youtube key function : by knuth

	def showYouTube(self):
		chipset = open("/proc/stb/info/chipset","r").readline()
		brandtype = open("/etc/.brandtype","r").readline()

		if "technomate" in brandtype:
			if "7356" in chipset or "7362" in chipset:
			...
			try:
				...
				self.session.open(MessageBox, _("The MyTube plugin is not installed!\nPlease install it."), type = MessageBox.TYPE_INFO,timeout = 10 )
