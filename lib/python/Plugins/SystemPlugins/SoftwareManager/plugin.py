...
class ImageBackup(Screen):
	...
	def createBackupJob(self):
		...
		task = Components.Task.PythonTask(job, _("Coping to USB Location..."))
		task.work = self.doBackup5
		task.weighting = 5

		### by knuth coping to usb
		task = Components.Task.ConditionTask(job, _("Coping to USB Location..."), timeoutCount=900)
		task.check = lambda: self.Stage5Completed
		task.weighting = 5

		task = Components.Task.PythonTask(job, _("Coping to files. Please wait..."))
		task.work = lambda: self.Stage6Completed
		task.weighting = 5
...
#		self.BackupDate = getImageVersionString() + '.' + getBuildVersionString() + '-' + strftime('%Y%m%d_%H%M%S', localtime())
		...
		JFFS2OPTIONS=" --disable-compressor=lzo --eraseblock=0x20000 -n -l --pagesize=0x800"
		UBINIZE='ubinize'
		...
		brand_type = open('/etc/.brandtype', 'r').readline()

		# by knuth
		if brand_type == "edision":
			if hw_type == "force1plus":
				hw_type = "optimussos3plus"
		elif hw_type in ( 'ios100hd', 'ios200hd', 'ios300hd' ):
			hw_type = hw_type.split()[0][:-2] ## ios100hd --> ios100

		self.MAINDEST = self.MAINDESTROOT + '/update/' + hw_type + '/cfe'
	...
	def doBackup2(self):
		...
		### by knuth
		if HardwareInfo().get_device_name() in ( 'tmnanooe' , 'tmnano2t', 'optimussos1' ):
			self.command = 'cat /dev/mtd1 > ' + self.WORKDIR + '/vmlinux.gz'
		elif HardwareInfo().get_device_name() in ( 'tm2toe', 'tmsingle', 'tmtwinoe', 'ios100hd', 'ios200hd', 'ios300hd', 'optimussos2', 'mediabox' ):
	...
	def doBackup5(self):
		## Debug Message Underline by knuth
		...
		if "edision" in brand_type:
			if hw_type == "force1plus":
				hw_type = "optimussos3plus"

		# do implrement : copy usb folder.
		self.copy_USB = config.plugins.configurationbackup.backuplocation.value + "/update/" + hw_type + "/cfe/"

		if not fileExists(self.copy_USB):
			makedirs(self.copy_USB,0644)
			print '[ImageManager]: makedirs -> copy_USB', self.copy_USB
		else:
			rmtree(self.copy_USB)
			print '[ImageManager]: ', self.copy_USB
			makedirs(self.copy_USB,0644)
			
		print '[ImageManager]:', self.MAINDEST

		copy(self.MAINDEST + '/oe_kernel.bin', self.copy_USB + '/oe_kernel.bin')
		copy(self.MAINDEST + '/oe_rootfs.bin', self.copy_USB + '/oe_rootfs.bin')

		for i in  range(0,30):
			sleep(1)
#			print '[ImageManager] : file copy delaying............', i

		# compare only filesystem.
		self.srcfilesize = os.path.getsize(self.MAINDEST + '/oe_rootfs.bin')
		self.dstfilesize = os.path.getsize(self.copy_USB + '/oe_rootfs.bin')

#		print "[ImageManager] : file size src dst", self.srcfilesize, self.dstfilesize
		if self.srcfilesize > self.dstfilesize:
			for i in range(0,10):
				sleep(1)
				print '[ImageManager] : not same filesize'
		else:
			print '[ImageManager] : same filesize'
			self.Stage5Complete()
...
class ImageRestore(Screen):
	...
	def doRestore(self,answer):
		if answer:
			...
			if HardwareInfo().get_device_name() in ("force1", "tm2tsuper", "optimussos1plus", "optimussos2plus", "tmnano2super", "force1plus", "force2solid", "force2", "tmnanose", "optimussosplus", "force2plus", "tmnanosecombo", "tmnanosem2", "force2eco","fusionhd" ):
				kernelMTD = "mtd2"
				kernelFILE = "oe_kernel.bin"
				rootMTD = "mtd0"
				rootFILE = "oe_rootfs.bin"
			elif HardwareInfo().get_device_name().startswith("tmnano", "tmnano2t", "ios300", "optimussos1",  "tmnanosuper"):
				kernelMTD = "mtd1"
				kernelFILE = "oe_kernel.bin"
				rootMTD = "mtd0"
				rootFILE = "oe_rootfs.bin"
